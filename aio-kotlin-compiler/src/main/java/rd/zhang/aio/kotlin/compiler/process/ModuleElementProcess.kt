 ...
 */
class ModuleElementProcess(private var element: ModuleElement,
    ...
    override fun process() {
        ...
        if (element.interceptorElements != null) {
            Collections.sort<InterceptorElement>(element.interceptorElements) { interceptorElement, t1 -> t1.priority!!.compareTo(interceptorElement.priority) }
            for (interceptor in element.interceptorElements!!) {
                when {
                //router
                    interceptor.type == 1 -> {
